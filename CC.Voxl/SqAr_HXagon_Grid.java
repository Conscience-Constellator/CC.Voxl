...
public class SqAr_HXagon_Grid extends Y_Stagrd
{
	...
	public static final double HXagon_Stagr=0.288675134594813/2;
	@Override
	public RL_LOc_Havr[][][] MAk_VrtXg(int X_LNg,int Y_LNg,int Z_LNg)
	{
		...
		double
			Voxl_Rad=Voxl_SIz/2,
			Y_Stagr=Voxl_SIz*HXagon_Stagr;
		for(int Z=0;
			Z<=Z_LNg;
			Z+=1)
		{
			VrtX2=VrtXg[Z];
			double Stagr_Distnc=Y_Stagr;
			for(int Y=0;
				Y<=Y_LNg;
				Y+=1,
				Stagr_Distnc=-Stagr_Distnc)
			{
				VrtX1=VrtX2[Y];
				int Stagr_Dr=1;
				for(int X=0;
					...
					Stagr_Dr=-Stagr_Dr)
				{
					VrtX1[X]=new RL_LOc_Havr_ConcrEt(this,
						X*Voxl_Rad,
						Y*Voxl_SIz+(Stagr_Distnc*Stagr_Dr),
						Z*Voxl_SIz);
				}
			}
		}
	...
	public SqAr_HXagon_Grid(RL_LOc_Havr Parnt,Object Loc_Sorc,double Voxl_SIz,byte[][][] IDg)
